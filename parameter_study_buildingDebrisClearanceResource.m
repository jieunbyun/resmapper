...
clearResourceWeightMultiply = [1.5 0.5];

for iParamTest = 1:length( clearResourceWeightMultiply )
    disp( ['Test ' num2str(iParamTest), '(/', num2str(length( clearResourceWeightMultiply )), ')'] )
    versionName = strcat('cb', num2str(iParamTest)); % clearance resource of building debris

    iClearResourceWeight = clearResourceWeight * clearResourceWeightMultiply(iParamTest); 
    roads = evalNClosureDaysByBuilding( roads, districts, averageBuildingLength_m, iClearResourceWeight, clearResource_day, roadsClearPriority );
