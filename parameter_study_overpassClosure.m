...
closureRatioOverpass = [0.5 0.3];

for iParamTest = 1:length( closureRatioOverpass )
    disp( ['Test ' num2str(iParamTest), '(/', num2str(length( closureRatioOverpass )), ')'] )
    versionName = strcat('oc', num2str(iParamTest)); % clearance resource of building debris

    iClosureRatioToRecoveryDays_overpass = closureRatioOverpass(iParamTest); 
    roads = evalNClosureDaysByOverpass( roads, iClosureRatioToRecoveryDays_overpass );
