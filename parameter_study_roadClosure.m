...
closureRatioTest = [0.75 0.25];

for iParamTest = 1:length( closureRatioTest )
    disp( ['Test ' num2str(iParamTest), '(/', num2str(length( closureRatioTest )), ')'] )
    versionName = strcat('rc', num2str(iParamTest)); % clearance resource of building debris

    iClosureRatioToRecoveryDays_roadDamage = closureRatioTest(iParamTest); 
    roads = evalClosureDaysByRoadDamage( roads, iClosureRatioToRecoveryDays_roadDamage );
